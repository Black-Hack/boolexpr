...
static void encode_cmsat(std::unordered_map<uint32_t, var_t> &idx2var,
                         Glucose::Solver &solver, bx_t bx) {
    auto xs = bx->support();
    ...
    for (size_t i = 0; i < xs.size(); ++i) solver.newVar();
...
soln_t Variable::_sat() const {
    auto self = shared_from_this();
    auto x = static_pointer_cast<Variable const>(self);
    return make_pair(true, point_t{{x, one()}});
}

soln_t Operator::_sat() const {
...
void Zero::sat_iter_init(sat_iter *it) const {
    it->one_soln = false;
    it->sat = false;
}
...
sat_iter::sat_iter() : sat{false} {}

sat_iter::sat_iter(bx_t const &bx) { bx->sat_iter_init(this); }

void sat_iter::get_soln() {
    point.clear();
...
bool sat_iter::operator==(sat_iter const &rhs) const { return sat == rhs.sat; }

bool sat_iter::operator!=(sat_iter const &rhs) const { return !(*this == rhs); }

point_t const &sat_iter::operator*() const { return point; }

sat_iter const &sat_iter::operator++() {
    if (one_soln) {
        sat = false;
        point.clear();
    } else {
